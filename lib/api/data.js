...
// 优先数据源：public/data/ais.csv.gz（31 天 / 2066 艘船完整数据）
const GZ_FILENAME = 'public/data/ais.csv.gz';
...
function parseCsvText(csvText, sourceLabel) {
  console.log('[data] parsing CSV text from', sourceLabel);
  // 手写逐行解析：避免 split(/\r?\n/) 一次性创建百万级行数组，
  // 降低峰值内存（Vercel 函数 1GB 上限）。
  const records = [];
  ...
  while (lineStart < n) {
    ...
    let line = csvText.slice(lineStart, lineEnd);
    ...
    if (lineNo === 0) {
      ...
      if (line.trim()) {
        ...
        colIdx = {
          ...
          sog:
            headers.indexOf('Speed Over Ground (SOG)') >= 0
              ? headers.indexOf('Speed Over Ground (SOG)')
              : headers.indexOf('SOG'),
  ...
  return mergeAcrossDayBoundary(records);
}

// 读取 .gz（gzip）CSV：先尝试 gunzip，失败说明文件可能已被上游解压，按明文解析。
function parseGzCsv(gzPath) {
  console.log('[data] reading gz CSV:', gzPath);
  const buf = fs.readFileSync(gzPath);
  let csvText;
  try {
    csvText = zlib.gunzipSync(buf).toString('utf-8');
  } catch (err) {
    csvText = buf.toString('utf-8');
  }
  return parseCsvText(csvText, gzPath);
}

function parseCsv(csvPath) {
  console.log('[data] reading CSV:', csvPath);
  const raw = fs.readFileSync(csvPath, 'utf-8');
  return parseCsvText(raw, csvPath);
}

// 同 MMSI 跨零点合并：以 MMSI 分组后，对相邻两天最后/最先两条记录，
// 若 |Δt| < 2×平均报告间隔 且 |COG 差| < 10° 且 |SOG 差| < 2kn，则视为同一条轨迹，
// 在日界处插入一个桥接记录（端点线性外推），消除人为时间大洞，避免下游
// 插值/穿越判断在日界误判断轨。桥接点以原记录派生，不改变真实报告。
function mergeAcrossDayBoundary(records) {
  ...
  for (const [, list] of byMmsi) {
    ...
    let gapSum = 0;
    for (let i = 1; i < list.length; i++) gapSum += list[i].timestamp - list[i - 1].timestamp;
    ...
    for (let i = 1; i < list.length; i++) {
      ...
      // 仅在「跨日」且「特征一致」时桥接，避免误连两条无关船
      if (curDate !== nxtDate && dt <= maxDt) {
        const cogDiff = angularDiff(cur.cog ?? 0, nxt.cog ?? 0);
        const sogDiff = Math.abs((cur.sog ?? 0) - (nxt.sog ?? 0));
        // 仅在「跨日」且「特征一致」时桥接；跨日本身已排除同日重复，
        // 故不再要求 dt > avgGap（两段时 avgGap==dt 会导致条件永不成立）。
        if (cogDiff < 10 && sogDiff < 2) {
          const midT = Math.floor((cur.timestamp + nxt.timestamp) / 2);
          const ratio = (midT - cur.timestamp) / dt;
          out.push({
            ...cur,
            lat: lerpNum(cur.lat, nxt.lat, ratio),
            lng: lerpNum(cur.lng, nxt.lng, ratio),
            sog: lerpNum(cur.sog ?? 0, nxt.sog ?? 0, ratio),
            cog: interpCog(cur.cog ?? 0, nxt.cog ?? 0, ratio),
            heading: lerpNum(cur.heading ?? cur.cog ?? 0, nxt.heading ?? nxt.cog ?? 0, ratio),
            timestamp: midT,
          });
        }
      }
...
// 航向角插值：对单位向量 (sinθ,cosθ) 插值后 atan2 还原，抗 0/360 环绕。
function interpCog(a, b, t) {
  ...
  let deg = (Math.atan2(sin, cos) * 180) / Math.PI;
  if (deg < 0) deg += 360;
  return deg;
}
...
function loadRecords() {
  ...
  try {
    // 0. 最高优先级数据源：public/data/ais.csv.gz（31 天完整数据）。
    // 改用旧的小样本可置 USE_SMALL_DATA=1 跳过此处（连同 data_10k / JSON）。
    if (!useSmall) {
      const gzPath = findFile([GZ_FILENAME]);
      if (gzPath) {
        if (isCacheValid(gzPath)) {
          console.log('[data] using cached gz:', gzPath);
          return cachedRecords;
        }
        console.log('[data] loading gz:', gzPath);
        cachedRecords = parseGzCsv(gzPath);
        setCache(cachedRecords, gzPath);
        console.log('[data] loaded from gz:', cachedRecords.length);
        return cachedRecords;
      }
    }
    ...
    if (!useSmall) {
      const jsonCandidates = [
        path.join(process.cwd(), 'lib', 'api', 'record1.json'),
        path.join('/var/task', 'lib', 'api', 'record1.json'),
        path.join(__dirname, 'record1.json'),
        path.join(process.cwd(), 'lib', 'api', 'records.json'),
        path.join('/var/task', 'lib', 'api', 'records.json'),
      ];
...
function getShipsLatest() {
  ...
  return Array.from(latestMap.values()).map((r) => ({
    ...
    lastIso: new Date(r.timestamp * 1000).toISOString(),
  }));
}
...
// ---- Vercel 远程数据源（函数包不打包大文件，运行时从静态资源拉取全量 gz）----
...
async function loadFromHttp() {
  ...
  if (!base) {
    throw new Error('[data] VERCEL_URL not set, cannot fetch remote data');
  }
  const url = /^https?:\/\//i.test(base)
    ? `${base}/data/ais.csv.gz`
    : `https://${base}/data/ais.csv.gz`;
  console.log('[data] fetching remote gz:', url);
  const res = await fetch(url, { headers: { accept: 'application/gzip,*/*' } });
  if (!res.ok) throw new Error(`[data] HTTP ${res.status} fetching ${url}`);
  const buf = Buffer.from(await res.arrayBuffer());
  let csvText;
  try {
    csvText = zlib.gunzipSync(buf).toString('utf-8');
  } catch (err) {
    // 静态服务器可能按明文 CSV 托管，直接按明文解析
    csvText = buf.toString('utf-8');
  }
  const records = parseCsvText(csvText, url);
  cachedRecords = records;
  cachedSourcePath = url;
  cachedSourceMTime = null;
  console.log('[data] loaded from remote:', records.length, 'records');
}

// Vercel 环境优先拉取静态全量 gz；失败或非 Vercel 走原同步加载链（data_10k / JSON / 内嵌）。
// 每个依赖 data.js 的 API handler 入口先 await ensureLoaded()。
async function ensureLoaded() {
  if (cachedRecords !== null) return cachedRecords;
  if (process.env.VERCEL === '1') {
    if (!httpLoading) {
      httpLoading = loadFromHttp().catch((err) => {
        console.error('[data] remote gz load failed:', err.message);
        httpLoading = null; // 允许下次请求重试
      });
    }
    await httpLoading;
    if (cachedRecords !== null) return cachedRecords;
